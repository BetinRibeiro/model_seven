...
def lista_clientes():
    ...
    if pagina <= 0:
        redirect(URL(args=1))
    total = db(db.pessoa.representante==usuario.id).count()
    ...
    registros = db(db.pessoa.representante==usuario.id).select(
      limitby=limites,orderby=~db.pessoa.id
      )
    consul=(request.args(1))
    if (consul):
        registros = db((db.pessoa.representante==usuario.id)&((db.pessoa.cpf.contains(consul))|(db.pessoa.razaosocial_nome.contains(consul)))).select(limitby=(0,paginacao))
...
def index():
    ...
    if usuario.tipo=="Representante":
        redirect(URL('lista_clientes',args=usuario.id))
    empresa = db.empresa(usuario.empresa)
    paginacao = 35
    if len(request.args) == 0:
        pagina = 1
